...
const [SnareNotes, setSnareNotes] = useState([true,false,true,false,false,false,false,false,false,false,false,false,false,false,false,true]);
const [test, settest] = useState();
const [lock, setlock] = useState(false);
const notesRef = useRef(SnareNotes);
notesRef.current = SnareNotes;
...
const handleClick =  (e) => {
    console.log(e)
    let notes_copy = []
   notes_copy = SnareNotes.slice()

    notes_copy[e] = !notes_copy[e];
   setSnareNotes(notes_copy)
...
// }, []);
   ...
   return (
    <div　ref={scheduleRef}>
                <div className="seq">
                    {new Array(32).fill().map((v, i) => (<div className={SnareNotes[i] ? "box_fill" : i % 4 === 0 ? "box_quarter_note" : "box_unfill"} key={i} onClick={() => handleClick(i)}></div>))}
                </div>
